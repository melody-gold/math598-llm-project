# LLM Transformer Project - Contributions Document:
=====================================================
I just listed the blocks I contributed to but want to note that what I put down are not things I did by myself, just the things I recall adding to/creating. 

#### Talia Kumar's Contributions:
1. Getting Data Block:
    - Helped find the data and clean the text on import
2. Tokenize Text Block:
    - Helped create the init function and the clean text function
    - Provided (mostly verbal/collaborative) help on the tokenizer and detokenizer
3. Attention Head Block:
    - Created the M_matrix function
4. Transformer Block Block (lol):
    - Helped create the init and forward functions
5. Save and Load Modules Block:
    - Created Save and Load modules functions
6. MLP:
    - Collaborated on the logic 
7. Py Testing:
    - Created test github action workflow and all of the tests
8. main.py:
    - Created everything inside of main.py
9. Edit final version of transformer.ipynb

#### Jordyn Graham's Contribution: 
Note: I did miss the first day of working on this project and joined the group late on the second work day in class. 
1. Tokenizer class 
    - I primarily worked on the vocab map, encoder, and decoder. To test it, I set up a test case using the raw data. 
    - In class I worked collaboratively on this block as to the steps and approach. 
2. MLP class
    - I worked collaboratively on the forward function and overall architecture. Did not write code but instead provided verbal comments and help.  
3. Transformer Block
    - Worked on in class and did not write any code but instead provided verbal collaboration. 
4. Training Loop 
    - Created training loop function to take in tokenized text and then split it for training and validation tests. 
    - After Melody debugged the training function, I added a plot for the loss function. 
5. Writeup.txt File 
    - Created this file for the final submission and wrote up initial specifically for architecture design choices and challenges. 

#### Dishita Sharma's Contribution:
1. Config
    - I pair programmed with Melody to set up the config portion of the code
2. Transformer Block 
    - Pair programmed with Melody to code the transformer block 
3. transformer model class 
    - Pair programmed to create the tranformer model class 
4. Generator function 
    - I wrote the entire code for the generator function 
5. Training Loop   
    - Helped with training loop - documentation 
6. pyproject.toml File 
    - Helped setup the pyproject.toml file 
7. .gitignore
8. README.md

#### Melody Goldanloo's Contribution:
1. Set up GitHub repo + Notebook
    - I have been writing all of the LaTeX and markdown in the notebook
    - Adding necessary documentation to repo: readme, toml,
2. Config and Tranformer Block
    - pair programmed with Dishita
3. Positional Embedding
    - Set that up 
4. Debugged and got the training loop running after it was implemented
    - there were a lot of dimension mismatch issues that I had to fix